...
from app.models import Execution,TestCases,TestSteps,Tasks,Element,Page,TaskLog
...
def createXml(taskid,excuteid):
    ...
    for case in cases:
        ...
        for ts in tss:
            ...
            ele = Element.query.filter((Element.name==ts.element).__and__(Element.pageid==page.id)).first() #step level
            if ele is not None:
